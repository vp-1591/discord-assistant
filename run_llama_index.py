...
class RAGAssistant:
    ...
    async def generate_refined_response(self, query_text: str, rag_response: str, history: List[str], summary: str = None, agent1_prompt: str = "", bot_name: str = None):
        ...
        refine_prompt = (
            ...
            "5. **Ограничения:** ЗАПРЕЩЕНО объяснять свою роль, упоминать важность свитков или давать напутствия 'продолжать поиск истины', если тебя об этом не просили напрямую. Просто дай ответ на вопрос.\n\n"
...
- **Сжатие:** Если новая информация дополняет или заменяет старую — обнови её. Удаляй временные детали и повторы.
...
Ответ только на русском языке (макс. 5 предложений).
"""
